...
Cypress.Commands.add("isValid", function(variable){
...
Cypress.Commands.add("compare2Arrays", function(arr1, arr2) {
...
Cypress.Commands.add("fillDatas", function(inputs){
	...
	if (inputs != undefined && inputs.length > 0)
		inputs.forEach(input => {
			switch (input.type) {
				case "text":
					cy.get('[ng-model="'+ input.selector +'"]').clear().type(input.value)
					break;
				case "select":
					if (input.value != undefined){
						cy.get('[ng-model="' + input.selector + '"]').first().click()
						.find('li[role="option"]').contains(input.value).first().click();
					}
...
Cypress.Commands.add("deleteByPos", function(url){
	switch(url.pos){
		case "first": {
			cy.get('a[ng-click="'+ url.delete)+'"]'.first().click();
			cy.get('button').contains('Đồng ý').click()
			  .get('div.toast-message').should('contain','Xóa thành công')
			break;
		}
		case "last":{
			cy.get('a[ng-click="'+ url.delete+'"]').last().click();
			cy.get('button').contains('Đồng ý').click();
			cy.wait('div.toast-message')
			  .get('div.toast-messag').should('contain','Xóa thành công')
			break;
		}
	}
})
